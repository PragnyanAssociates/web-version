...
import StudentSyllabusNavigator from "./StudentSyllabusScreen"; // unchanged
import TeacherSyllabusNavigator from "./TeacherSyllabusScreen"; // unchanged
import AdminSyllabusScreen from "./AdminSyllabusScreen"; // unchanged
...
const SyllabusScreen = ({ navigation }) => {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-teal-50/30 to-cyan-50/30 flex flex-col justify-center items-center relative overflow-hidden">
        ...
        <div className="relative z-10 text-center">
          ...
          <p className="text-gray-600">Please wait while we set up your syllabus portal...</p>
        </div>
      </div>
    );
  }
  ...
  if (user.role === "student")
    return <StudentSyllabusNavigator navigation={navigation} />;

  if (user.role === "teacher")
    return <TeacherSyllabusNavigator navigation={navigation} />;

  if (user.role === "admin")
    return <AdminSyllabusScreen navigation={navigation} />;

  // Fallback for unsupported roles
  return (
    <ScreenLayout title="Access Restricted" subtitle="Syllabus portal is not available for your role">
      <div className="text-center py-16">
        <div className="backdrop-blur-sm bg-white/80 rounded-3xl shadow-xl border border-white/20 p-8 max-w-md mx-auto">
          ...
          <p className="text-gray-500 mb-4">
            The syllabus portal is not available for users with your current role.
          </p>
